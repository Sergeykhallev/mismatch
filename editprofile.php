...
if (!isset($_SESSION['user_id'])) {
    echo '<p class="login">Пожалуйста <a href="login.php">авторизоваться</a> чтобы получить доступ к этой странице.</p>';
    exit();
}
else {
    echo('<p class="login">Вы вошли в систему как ' . $_SESSION['username'] . '. <a href="logout.php">выйти</a>.</p>');
}
...
if (isset($_POST['submit'])) {
    // Возьмите данные профиля из суперглобального массива POST
    ...
    // Проверьте и переместите загруженный файл изображения, если это необходимо
    if (!empty($new_picture)) {
        if ((($new_picture_type == 'image/gif') || ($new_picture_type == 'image/jpeg') || ($new_picture_type == 'image/pjpeg') ||
                ($new_picture_type == 'image/png')) && ($new_picture_size > 0) && ($new_picture_size <= MM_MAXFILESIZE) &&
            ($new_picture_width <= MM_MAXIMGWIDTH) && ($new_picture_height <= MM_MAXIMGHEIGHT)) {        if ($_FILES['file']['error'] == 0) {          // Переместите файл в целевую папку для загрузки
            $target = MM_UPLOADPATH . basename($new_picture);
            if (move_uploaded_file($_FILES['new_picture']['tmp_name'], $target)) {
                // Перемещение нового файла изображения прошло успешно, теперь убедитесь, что все старые изображения удалены
            ...
            // Новый файл изображения недействителен, поэтому удалите временный файл и установите флаг ошибки
            @unlink($_FILES['new_picture']['tmp_name']);
            $error = true;        echo '<p class="error">Ваше изображение должно быть в формате GIF, JPEG или PNG размером не более ' . (MM_MAXFILESIZE / 1024) .
                ' KB и ' . MM_MAXIMGWIDTH . 'x' . MM_MAXIMGHEIGHT . 'пикселей по размеру.</p>';      }
    }
    ...
    if (!$error) {
        if (!empty($first_name) && !empty($last_name) && !empty($gender) && !empty($birthdate) && !empty($city) && !empty($state)) {
            // Устанавливайте столбец "Изображение" только в том случае, если есть новое изображение
            if (!empty($new_picture)) {
...
} // Окончание проверки для отправки формы
else {
...
<form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <fieldset>
        ...
        <select id="gender" name="gender">
            <option value="M" <?php if (!empty($gender) && $gender == 'М') echo 'selected = "selected"'; ?>>Мужской</option>
            <option value="F" <?php if (!empty($gender) && $gender == 'Ж') echo 'selected = "selected"'; ?>>Женский</option>
        </select><br />
